...
public class String_ {
    ...
    // Sneaky private mutable state, to cache the result of a maxChar() computation!
    private char maxCharIs;
    private boolean maxCharSet = false;
    
    /** Return the character in the string with the highest ASCII code.
     * @return that character, or 0 for an empty string
     */
    public char maxCharFast () {
    	if (maxCharSet) {
    		// We already know the answer, so return it.
    		return maxCharIs;
    	} else {
	    	char ch = 0;
	    	...
	    	// Record both the answer and the fact that we know it.
	    	maxCharIs = ch;
	    	maxCharSet = true;
	    	return ch;
    	}
